         ...
         `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommWrite")`
{
    ...
    while(bufIndex < size)
    {
        `$INSTANCE_NAME`_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         `=ReentrantKeil($INSTANCE_NAME . "_CyBtldrCommRead")`
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  `$INSTANCE_NAME`_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(`$INSTANCE_NAME`_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(`$INSTANCE_NAME`_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional `$INSTANCE_NAME`_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != `$INSTANCE_NAME`_PACKET_EOP)
            {
                CyDelay(`$INSTANCE_NAME`_WAIT_EOP_DELAY);
            }
        }
