...
uint8_t UART_1_initVar = 0u;
    ...
    volatile uint8_t UART_1_txBuffer[UART_1_TX_BUFFER_SIZE];
    ...
    uint8_t UART_1_errorStatus = 0u;
    volatile uint8_t UART_1_rxBuffer[UART_1_RX_BUFFER_SIZE];
    volatile uint8 UART_1_rxBufferRead  = 0u;
    volatile uint8 UART_1_rxBufferWrite = 0u;
    volatile uint8_t UART_1_rxBufferLoopDetect = 0u;
    volatile uint8_t UART_1_rxBufferOverflow   = 0u;
    #if (UART_1_RXHW_ADDRESS_ENABLED)
        volatile uint8_t UART_1_rxAddressMode = UART_1_RX_ADDRESS_MODE;
        volatile uint8_t UART_1_rxAddressDetected = 0u;
...
void UART_1_Init(void) 
{
        ...
        UART_1_WriteControlRegister( \
            (UART_1_ReadControlRegister() & (uint8_t)~UART_1_CTRL_PARITY_TYPE_MASK) | \
            (uint8_t)(UART_1_PARITY_TYPE << UART_1_CTRL_PARITY_TYPE0_SHIFT) );
...
void UART_1_Stop(void) 
{
    ...
    #if (UART_1_RX_ENABLED || UART_1_HD_ENABLED)
        UART_1_RXBITCTR_CONTROL_REG &= (uint8_t) ~UART_1_CNTR_ENABLE;
    #endif /* (UART_1_RX_ENABLED || UART_1_HD_ENABLED) */

    #if (UART_1_TX_ENABLED)
        #if(!UART_1_TXCLKGEN_DP)
            UART_1_TXBITCTR_CONTROL_REG &= (uint8_t) ~UART_1_CNTR_ENABLE;
    ...
    #if (UART_1_RX_ENABLED || UART_1_HD_ENABLED)
        UART_1_RXSTATUS_ACTL_REG  &= (uint8_t) ~UART_1_INT_ENABLE;
    ...
    #if (UART_1_TX_ENABLED)
        UART_1_TXSTATUS_ACTL_REG &= (uint8_t) ~UART_1_INT_ENABLE;
...
void  UART_1_WriteControlRegister(uint8_t control) 
{
    ...
    *******************************************************************************/
    void UART_1_ClearRxBuffer(void) 
    {
        ...
        /* Clear the HW FIFO */
        enableInterrupts = CyEnterCriticalSection();
        UART_1_RXDATA_AUX_CTL_REG |= (uint8_t)  UART_1_RX_FIFO_CLR;
        UART_1_RXDATA_AUX_CTL_REG &= (uint8_t) ~UART_1_RX_FIFO_CLR;
        CyExitCriticalSection(enableInterrupts);
    ...
    *******************************************************************************/
                                                        ...
                                                        
    {
                ...
                uint8_t tmpCtrl;
                tmpCtrl = UART_1_CONTROL_REG & (uint8_t)~UART_1_CTRL_RXADDR_MODE_MASK;
                tmpCtrl |= (uint8_t)(addressMode << UART_1_CTRL_RXADDR_MODE0_SHIFT);
    ...
    *******************************************************************************/
    void UART_1_PutString(const char8 string[]) 
    {
        ...
        if(UART_1_initVar != 0u)
        {
            ...
            {
                UART_1_PutChar((uint8_t)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
    void UART_1_ClearTxBuffer(void) 
    {
        ...
        enableInterrupts = CyEnterCriticalSection();
        /* Clear the HW FIFO */
        UART_1_TXDATA_AUX_CTL_REG |= (uint8_t)  UART_1_TX_FIFO_CLR;
        UART_1_TXDATA_AUX_CTL_REG &= (uint8_t) ~UART_1_TX_FIFO_CLR;
        CyExitCriticalSection(enableInterrupts);
    ...
    void UART_1_SendBreak(uint8_t retMode) 
    {
        ...
        if(UART_1_initVar != 0u)
        {
                ...
                (retMode == UART_1_SEND_WAIT_REINIT) )
            {
                ...
                while(((uint8_t)~tmpStat & UART_1_TX_STS_COMPLETE) != 0u);
            }
            ...
            {
                UART_1_WriteControlRegister(UART_1_ReadControlRegister() &
                                              (uint8_t)~UART_1_CTRL_HD_SEND_BREAK);
            }
